...
async function makeRequest(endpoint, options = {}) {
  ...
  try {
    ...
    if (error.code === 'ECONNREFUSED') {
      throw new Error(
        "❌ Cannot connect to development server. Make sure it's running on " + API_BASE
      )
    }
    throw error
  }
...
async function testTMDBDirectly() {
  ...
  try {
    ...
    console.log(
      `   ✅ TMDB search working - found ${searchData.results.length} results for "inception"`
    )
    ...
    console.log(
      `   ✅ TMDB trending working - found ${trendingData.results.length} trending movies`
    )
...
async function testDatabaseManager() {
  ...
  try {
    ...
    console.log(
      `   Available databases: ${databases.availableDatabases.map(db => db.name).join(', ')}`
    )
...
async function testMoviesAPI() {
  ...
  try {
    ...
    const smartResponse = await makeRequest(
      '/api/movies?smart=true&realtime=true&database=tmdb&limit=5'
    )
