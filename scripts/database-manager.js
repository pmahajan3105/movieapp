...
async function listDatabases() {
  ...
  data.availableDatabases.forEach((db, index) => {
    ...
    const quality = Math.round(
      (db.dataQuality.metadata +
        db.dataQuality.images +
        db.dataQuality.freshness +
        db.dataQuality.coverage) /
        4
    )
    ...
    console.log(
      `     Rate Limit: ${db.rateLimit.requestsPerSecond}/sec, ${db.rateLimit.requestsPerDay}/day`
    )
...
async function getDatabaseInfo(databaseId) {
  ...
  const avgQuality = Math.round(
    (database.dataQuality.metadata +
      database.dataQuality.images +
      database.dataQuality.freshness +
      database.dataQuality.coverage) /
      4
  )
...
async function calculateCosts() {
  ...
  scenarios.forEach(scenario => {
    console.log(
      `📊 ${scenario.name.toUpperCase()} (${scenario.requests.toLocaleString()} requests/month)`
    )
    console.log('='.repeat(60))
  ...
  console.log('   • Consider rate limits when choosing external APIs')
...
async function main() {
  ...
  if (!command || command === 'help') {
    ...
    console.log('   node scripts/database-manager.js info tmdb')
