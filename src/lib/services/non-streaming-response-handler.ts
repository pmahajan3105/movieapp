...
export class NonStreamingResponseHandler {
  ...
  static async processNonStreamingResponse(
    ...
    try {
      ...
      if (fullHistory.length >= 4) { // At least 2 user messages and 2 AI responses
        ...
        if (conversationLength > 100) { // Meaningful conversation
      ...
      if (shouldExtractPreferences) {
        ...
        try {
          extractedPreferences = await PreferenceExtractionService.extractPreferencesFromConversation(
            fullHistory,
            anthropicApiKey
          )
          ...
          if (extractedPreferences && Object.keys(extractedPreferences).length > 0) {
            ...
            return NextResponse.json({
              ...
              completionMessage: "Great! I've learned about your movie preferences. You can now get personalized recommendations!"
            })
          }
