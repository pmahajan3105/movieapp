...
export class EmotionalJourneyMapper {
  ...
  calculateEmotionalCompatibility(
    ...
    try {
      ...
      const compatibilityScore = (
        ...
        intensityMatch * 0.25 +
        journeyFit * 0.25 +
        catharticPotential * 0.2
      )

      const explanation = this.generateCompatibilityExplanation(
        movieJourney,
        userRequest,
        { moodAlignment, intensityMatch, journeyFit, catharticPotential }
      )
  ...
  analyzeUserEmotionalPatterns(moodHistory: MoodHistoryEntry[]): EmotionalPreference[] {
    try {
      ...
      const patterns: Record<string, { count: number; satisfaction: number; preferences: Record<string, number> }> = {}
    ...
    try {
      const similarities = candidateJourneys.map(candidate => {
        ...
        return {
          ...
          similarity,
          explanation: this.generateSimilarityExplanation(referenceJourney, candidate.journey, similarity)
        }
      })
   ...
   */
  private async performAIEmotionalAnalysis(request: EmotionalAnalysisRequest): Promise<Partial<EmotionalJourney>> {
...
CONTEXT: The user is currently in a ${request.userMoodContext} mood. Consider how this film's emotional journey would align with or complement this emotional state.`
  ...
  private extractEmotionalPattern(text: string): EmotionalJourney['overallPattern'] {
    ...
    if (lowerText.includes('rollercoaster') || lowerText.includes('volatile')) return 'rollercoaster'
  ...
  private extractEmotionalBeats(text: string): EmotionalBeat[] {
    ...
    if (beats.length === 0) {
      beats.push(
        { timestamp: 0, emotion: 'opening', intensity: 0.4, description: 'Opening mood establishment' },
        { timestamp: 25, emotion: 'development', intensity: 0.6, description: 'Emotional development' },
        { timestamp: 75, emotion: 'climax', intensity: 0.9, description: 'Emotional climax' },
        { timestamp: 100, emotion: 'resolution', intensity: 0.5, description: 'Emotional resolution' }
      )
    }
  ...
  private findClimacticMoment(beats: EmotionalBeat[], text: string): EmotionalBeat {
    ...
    let climacticBeat = beats.reduce((max, beat) => 
      beat.intensity > max.intensity ? beat : max, 
      beats[0] || { timestamp: 75, emotion: 'climax', intensity: 0.9, description: 'Emotional peak' }
    )
    
    // Enhance description based on analysis text
    const climaxKeywords = ['climax', 'peak', 'highest', 'maximum', 'intense moment']
    const climaxSentences = text.split('.').filter(sentence => 
      climaxKeywords.some(keyword => sentence.toLowerCase().includes(keyword))
    )
  ...
  private findResolution(beats: EmotionalBeat[], text: string): EmotionalBeat {
    ...
    const resolutionSentences = text.split('.').filter(sentence => 
  ...
  private enhanceWithPatternRecognition(
    ...
    const patternTemplate = EMOTIONAL_PATTERNS[pattern.toUpperCase() as keyof typeof EMOTIONAL_PATTERNS]
  ...
  private calculateMoodAlignment(journey: EmotionalJourney, request: EmotionalCompatibilityRequest): number {
  ...
  private calculateIntensityMatch(journey: EmotionalJourney, request: EmotionalCompatibilityRequest): number {
  ...
  private calculateJourneyFit(journey: EmotionalJourney, request: EmotionalCompatibilityRequest): number {
  ...
  private calculateCatharticPotential(journey: EmotionalJourney, request: EmotionalCompatibilityRequest): number {
  ...
  private generateCompatibilityExplanation(
    ...
    scores: { moodAlignment: number; intensityMatch: number; journeyFit: number; catharticPotential: number }
  ): string {
  ...
  private calculateEmotionalSimilarity(journey1: EmotionalJourney, journey2: EmotionalJourney): number {
    ...
    const beatSimilarity = this.calculateBeatSimilarity(journey1.emotionalBeats, journey2.emotionalBeats)
    
    return (
      patternSimilarity * 0.4 +
      intensitySimilarity * 0.3 +
      beatSimilarity * 0.3
    )
  }
  ...
  private generateSimilarityExplanation(journey1: EmotionalJourney, journey2: EmotionalJourney, similarity: number): string {
  ...
  private calculateIntensityTolerance(moodHistory: MoodHistoryEntry[], mood: string): number {
    const moodEntries = moodHistory.filter(entry => 
      entry.reportedMood === mood || entry.inferredMood === mood
    )
  ...
  private createFallbackEmotionalJourney(): EmotionalJourney {
    return {
      ...
      climacticMoment: { timestamp: 75, emotion: 'climax', intensity: 0.8, description: 'Peak moment' },
      resolution: { timestamp: 100, emotion: 'resolution', intensity: 0.5, description: 'Story conclusion' },
      intensityScore: 0.6,
