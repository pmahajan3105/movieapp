...
const addToWatchlistSchema = z
  ...
  .transform(data => {
    const movieId = data.movie_id || data.movieId
    if (!movieId) {
...
const updateWatchlistSchema = z
  ...
  .transform(data => {
    ...
    return { movieId, watched: data.watched, rating: data.rating, notes: data.notes }
  })
...
export const GET = withAuthAndErrorHandling(async (request: NextRequest, { user }) => {
...
export const POST = withAuthAndErrorHandling(async (request: NextRequest, { user }) => {
  const supabase = await createServerClient()
  const body = await withValidation(request, addToWatchlistSchema)

  let movieId = body.movieId // Now guaranteed to exist
...
export const PATCH = withAuthAndErrorHandling(async (request: NextRequest, { user }) => {
  const supabase = await createServerClient()
  const { movieId, watched, rating, notes } = await withValidation(request, updateWatchlistSchema)
